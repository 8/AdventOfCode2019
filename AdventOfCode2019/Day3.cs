...
namespace AdventOfCode2019
{
  public class Day3
  {
    ...
    static Point[] Collect(Wire wire)
    {
      ...
      return allPoints.Distinct().ToArray();
    }

    static int GetDistance(Point p)
      => Math.Abs(p.X) + Math.Abs(p.Y);

    static int GetDistance(Wire wire1, Wire wire2)
    {
    ...
    public void Day3_GetDistance(string line1, string line2, int expectedDistance)
    {
      ...
      var distance = GetDistance(wire1, wire2);
...
";
      ...
      var wires = GetWires(lines).ToArray();

      var distance = GetDistance(wires[0], wires[1]);
