...
namespace AdventOfCode2019
{
  public class Day3
  {
    ...
    static IEnumerable<Move> GetMoves(string line)
    {
      static Move Parse(string part)
      {
        var direction = part[0] switch
        {
          'R' => Direction.Right,
          'L' => Direction.Left,
          'U' => Direction.Up,
        ...
        return new Move { Direction = direction, Value = value };
      }

      var parts = line.Split(',', StringSplitOptions.RemoveEmptyEntries);

      return parts.Select(p => Parse(p));
    }

    static Wire GetWire(string line)
      => new Wire { Moves = GetMoves(line).ToArray() };

    static IEnumerable<Wire> GetWires(string text)
    {
      var sr = new StringReader(text);
      string line;
      while ((line = sr.ReadLine()) != null)
      {
        yield return GetWire(line);
      }
    }

    struct Point { public int X; public int Y; }

    static Point GetPoint(Move move)
      => move.Direction switch
      {
        Direction.Down => new Point { X = 0, Y = -move.Value },
        Direction.Left => new Point { X = -move.Value, Y = 0 },
        Direction.Right => new Point { X = move.Value, Y = 0 },
        Direction.Up => new Point { X = 0, Y = move.Value },
      };

    static Point[] Visit(Point start, Point direction)
    {
      var points = new List<Point>();
      points.Add(start);

      for (int x = 0; x < direction.X; x++)
        points.Add(new Point { X = start.X + x + 1, Y = start.Y });

      for (int x = 0; x > direction.X; x--)
        points.Add(new Point { X = start.X + x - 1, Y = start.Y });

      for (int y = 0; y < direction.Y; y++)
        points.Add(new Point { X = start.X, Y = start.Y + y + 1 });
    ...
    static Point[] Collect(Wire wire)
    {
      var allPoints = new List<Point>();

      var lastPos = new Point { X = 0, Y = 0 };
      allPoints.Add(lastPos);
      foreach (var move in wire.Moves)
      {
        var points = Visit(lastPos, GetPoint(move));
        allPoints.AddRange(points.Skip(1));
    ...
    static int GetLowestDistance(Wire wire1, Wire wire2)
    {
      var points1 = Collect(wire1);
      var points2 = Collect(wire2);

      var intersections = points1.Intersect(points2).ToArray();

      var result = intersections
        .Where(p => !(p.X == 0 && p.Y == 0))
        .OrderBy(p => GetDistance(p))
        .First();
    ...
    public void Day3_Parse_Line(string line)
    {
      var wire = GetWire(line);

      var points = Collect(wire);

      Console.Write($"points.Length: {points.Length}");
...
";
    ...
    {
      var wires = GetWires(_lines).ToArray();

      var distance = GetLowestDistance(wires[0], wires[1]);

      Console.WriteLine($"distance: {distance}");
    }

    static int GetLowestSteps(Wire wire1, Wire wire2)
    {
      var points1 = Collect(wire1);
      var points2 = Collect(wire2);
      ...
      static int GetIndex(Point[] points, Point point)
        => points.Select((p, i) => new { Point = p, Index = i })
        .Where(item => item.Point.X == point.X && item.Point.Y == point.Y)
        .Single().Index;

      var steps = intersections
        .Select(p => new { Point = p, Steps = GetIndex(points1, p) + GetIndex(points2, p) })
        .OrderBy(item => item.Steps)
        .Where(i => !(i.Point.X == 0 && i.Point.Y == 0))
        .ToArray();
    ...
    public void Day3_Steps(string line1, string line2, int expectedSteps)
    {
      var wire1 = GetWire(line1);
      var wire2 = GetWire(line2);
      var result = GetLowestSteps(wire1, wire2);
    ...
    public void Day3_Part2()
    {
      ...
      var steps = GetLowestSteps(wires[0], wires[1]);
